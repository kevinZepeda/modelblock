...
require_once(base_path() . '/app/Modules/invoicr/req/autoload.php');
...
class invoicr extends FPDF_rotation 
{
	...
	*******************************************************************************/
	...
	function addItem($item,$description,$quantity,$vat,$price,$discount=0,$total)
	{
		...
		$p['vat']			= $vat;
		if(is_numeric($vat)) {
			$p['vat']		= sprintf($this->currency, number_format($vat,2,$this->referenceformat[0],$this->referenceformat[1]));
		} 
		$p['quantity'] 		= $quantity;
		$p['price']			= $price;
		$p['total']			= $total;
		...
		if($discount!==false) {
			$this->firstColumnWidth = 80;
			$p['discount'] = $discount;
			if(is_numeric($discount)) {
				$p['discount']	= sprintf($this->currency, number_format($discount,2,$this->referenceformat[0],$this->referenceformat[1]));
			}
	...
	function addTotal($name,$value,$colored=0)
	{
		$t['name']			= $name;
		$t['value']			= $value;
		if(is_numeric($value) && $colored == 0) {
			$t['value']			= sprintf($this->currency, number_format($value,2,$this->referenceformat[0],$this->referenceformat[1]));
			$t['value']	 		= ($t['value']>0)?'+'.$t['value']:$t['value'];
	...
	function render($name='',$destination='')
	{
		...
		$this->Body();
		$this->AliasNbPages();
		$output = $this->Output($name,$destination);

		if(strtoupper($destination) == 'S'){
			return $output;
		}
	...
	*******************************************************************************/
	function Header()
	{
	    ...
	    //Number
	    if(strtolower($this->title) == 'invoice'){
		...
		//Date
		if(strtolower($this->title) == 'quote'){
			$this->Cell($positionX,$lineheight);
			$this->SetFont($this->font,'B',9);
			$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
			$this->Cell(32,$lineheight,iconv("UTF-8", "ISO-8859-1",strtoupper($this->l['quote_date'])).':',0,0,'L');	
			$this->SetTextColor(50,50,50);
			$this->SetFont($this->font,'',9);
			$this->Cell(0,$lineheight,$this->date,0,1,'R');
		}else if(strtolower($this->title) != 'recurring'){
			$this->Cell($positionX,$lineheight);
			$this->SetFont($this->font,'B',9);
			$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
		...
		//Due date
		if($this->due && strtolower($this->title) != 'quote') 
		{
			$this->Cell($positionX,$lineheight);
			$this->SetFont($this->font,'B',9);
			$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
		...
		if($this->PageNo()==1) 
		{
			...
			if(!empty($this->to)){
				if($this->title =='CREDIT NOTE'){
					$this->Cell(0,$lineheight,strtoupper($this->l['credit_note']),0,0,'L');
				}else{
	...
	{	
		...
		if($this->items) {
			foreach($this->items as $item) 
			{
				...
				// $this->Cell($this->columnSpacing,$cHeight,'',0,0,'L',0);  
				$this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252', sprintf($this->currency, number_format($item['total'],2,$this->referenceformat[0],$this->referenceformat[1]))),0,0,'C',1);
				$this->Ln();
		...
		if($this->totals) 
		{
			foreach($this->totals as $total) 
			{
				...
				{
					$this->Cell($width_other,5,'',0,0,'L',0);
					$this->Cell($this->columnSpacing,5,'',0,0,'L',0);
				}
				$this->Cell($this->columnSpacing,5,'',0,0,'L',0);
				if($total['colored']) 
				{
					$this->SetTextColor(255,255,255);
					if($colour_iterator < 3){
						$this->SetFillColor($this->color[0],$this->color[1] + $colour_index[$colour_iterator],$this->color[2]);
					}else{
				...
				if($total['colored']){
					$this->SetFont($this->font,'bU',6.5);
				}else{
					$this->SetFont($this->font,'',7);
				}
				//$this->Cell(1,5,'',0,0,'L',1);
				//$this->Cell($width_other-1,5,iconv('UTF-8', 'windows-1252',$total['name']),0,0,'L',1);
				//Fixed Version..
				$this->Cell($width_other,5,iconv('UTF-8', 'windows-1252',$total['name']),0,0,'L',1);
				//End of Fixed
				$this->Cell($this->columnSpacing,5,'',0,0,'L',0);
				...
				$this->Cell($width_other,5,iconv('UTF-8', 'windows-1252',$total['value']),0,0,'C',1);
				$this->Ln();
				$this->Ln($this->columnSpacing);
			}
		}
