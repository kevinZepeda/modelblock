...
<body class="login-page board" style="background-color:#eaeaea;">
  ...
  <div class="login-box">
    ...
    <div class="login-box-body">
      <p class="login-box-msg">{{trans('auth.password_reset.login_welcome')}}</p>
      <form role="form" method="POST" action="{{ url('/password/email') }}">
        ...
        <div class="form-group has-feedback">
          <input type="email" class="form-control" name="email" value="{{ old('email') }}" placeholder="{{trans('auth.password_reset.input.email')}}"/>
          <span class="glyphicon glyphicon-envelope form-control-feedback"></span>
        </div> 

        <div class="row" style="margin:0px;">
          <div class="col-xs-12">    
            <button type="submit" class="btn btn-primary btn-block btn-flat">{{trans('auth.password_reset.button.reset')}}</button>
          </div><!-- /.col -->
          <div class="col-xs-2">    
             &nbsp;
          </div><!-- /.col -->          
        ...
        <div class="row" style="margin-left:0px;text-align:center;">
          <div class="col-xs-12">
            <a  href="{{ url('/auth/login') }}">{{trans('auth.password_reset.button.remembered')}}</a>
          </div><!-- /.col -->
        </div>
      </form>
