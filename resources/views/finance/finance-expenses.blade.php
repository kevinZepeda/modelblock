                                ...
                                <tr role="row" class="odd" id="i{{$draft->id}}">
                                    <td>
                                        @if(!empty($draft->name))
                                            {{$draft->name}}
                                        @else
                                            <div style="color:#ccc;">{{trans('finance.expense.uncategorized')}}</div>
                                        @endif
                                    </td>
                                    <td>   
                                    ...
                                    <td>
                                        @if(!empty($draft->invoice_date))
                                            ...
                                            ?>
                                        @else
                                            <div style="color:#ccc;">{{trans('finance.expense.no_date')}}</div>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="btn-group">
                                            <div class="btn-group">
                                                <button onclick="location.href='{{ url('/office/finance/expense/'.$draft->id) }}'" type="submit" class="btn btn-xs">{{trans('finance.expense.button.edit')}}</button>
                                                <button type="button" class="btn btn-xs dropdown-toggle" data-toggle="dropdown">
                                                    <span class="caret"></span>
                                                    <span class="sr-only">Toggle Dropdown</span>
                                                </button>
                                                <ul class="dropdown-menu" role="menu" style="left:-50px;">
                                                    <li><a href="#" class="delete-invoice" id="{{$draft->id}}">{{trans('finance.expense.button.delete_permanently')}}</a></li>
                                                </ul>
                                            </div>
