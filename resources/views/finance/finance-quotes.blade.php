                            ...
                            <thead>
                                <tr role="row">
                                    <th class="sorting_asc" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="" style="width: 40%;">
                                        {{trans('finance.quote.col.customer')}}
                                    </th>
                                ...
                                <tr role="row" class="odd" id="i{{$quote->id}}">
                                    <td>
                                        @if(!empty($quote->customer_name))
                                            {{$quote->customer_name}}
                                        @else
                                            <div style="color:#ccc;">{{trans('finance.quote.not_assigned')}}</div>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <button onclick="location.href='{{ url('/office/finance/quote/'.$quote->id) }}'" type="submit" class="btn btn-xs">Edit</button>
                                            <button type="button" class="btn btn-xs dropdown-toggle" data-toggle="dropdown">
                                                <span class="caret"></span>
                                                <span class="sr-only">Toggle Dropdown</span>
                                            </button>
                                            <ul class="dropdown-menu" role="menu" style="left:-50px;">
                                                <li><a download href="{{ url('/api') }}?event=download_invoice&_token={{ csrf_token() }}&invoice_id={{$quote->id}}">{{trans('finance.quote.button.download')}}</a></li>
                                                <li><a href="#" class="invoice-clone" id="{{$quote->id}}">{{trans('finance.quote.button.clone_as_draft')}}</a></li>
                                                <li class="divider"></li>
                                                <li><a href="#" class="delete-invoice" id="{{$quote->id}}">{{trans('finance.quote.button.delete_permanently')}}</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
